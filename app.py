...
# Une fiche "la methode, pas a pas" par fiche de cours (voir methodes.py).
# Elle ajoute l'onglet Methode et alimente l'aide-memoire.
try:
    import methodes
    methodes.appliquer(BLOCS)
except ImportError:
    pass
# Fonctions ajoutees : videos, revision espacee, mode controle, aide-memoire,
# entrainement illimite, corrige progressif. Absent = application inchangee.
try:
    import options
except ImportError:
    options = None
...
# une progression enregistree avant l'ajout des options n'a pas ces cles
for _cle, _vide in (("videos", {}), ("srs", {}), ("controles", [])):
    P.setdefault(_cle, _vide)
...
PAGES = ["🏠 Tableau de bord", PAGE_COURS, "🎯 Quiz interactif"]
if options is not None:
    # Les libelles restent fixes : Streamlit retrouve ainsi la page choisie
    # d'une execution a l'autre. Le compteur s'affiche sous le menu.
    PAGES += ["↻ À revoir", "◷ Mode contrôle",
              "∞ Entraînement illimité", "∑ Aide-mémoire"]
PAGES += ["📐 Calculateur d'ajustements ISO",
          "🔧 Calculateurs RDM",
          "🧱 Base matériaux",
          "📊 Ma progression"]
...
if options is not None:
    _dues = len(options.srs_dues(P))
    if _dues:
        st.sidebar.caption(f"↻ {_dues} carte(s) à réviser aujourd'hui")
...
# ===========================================================================
    ...
    noms_onglets = ["📖 Cours", "📐 Formules", "🏭 Cas industriel",
                    "✍️ Exercice", "✅ Corrigé"]
    if fiche.get("methode"):
        noms_onglets.insert(2, "🧭 Méthode")
    if options is not None:
        noms_onglets.append("▶ Vidéos")
    onglets = st.tabs(noms_onglets)
    O = dict(zip(noms_onglets, onglets))
    ...
    with O["✅ Corrigé"]:
        if options is not None:
            options.corrige_progressif(fiche.get("corrige", ""), cle, afficher_contenu)
        else:
            afficher_contenu(fiche.get("corrige", ""))
    for _nom in noms_onglets:
        if _nom.startswith("▶ Vidéos"):
            with O[_nom]:
                options.zone_videos(P, sauver_progression, cle, fiche.get("titre", ""))
...
# ===========================================================================
# PAGES AJOUTÉES : révision espacée, contrôle, entraînement, aide-mémoire
# ===========================================================================

elif options is not None and PAGE == "↻ À revoir":
    options.page_revoir(P, sauver_progression, qz)

elif options is not None and PAGE == "◷ Mode contrôle":
    options.page_controle(P, sauver_progression, qz)

elif options is not None and PAGE == "∞ Entraînement illimité":
    options.page_entrainement(P, sauver_progression, iso286, mat)

elif options is not None and PAGE == "∑ Aide-mémoire":
    options.page_memo(BLOCS, afficher_contenu)
        ...
        if i < len(questions):
            ...
            if not st.session_state.quiz_valide:
                if st.button("Valider", type="primary", disabled=choix is None):
                    ...
                    # une question ratee devient une carte de revision espacee ;
                    # une question reussie fait monter la carte d'un cran
                    if options is not None:
                        options.srs_maj(P, sauver_progression, q["uid"], juste,
                                        q["question"][:80])
